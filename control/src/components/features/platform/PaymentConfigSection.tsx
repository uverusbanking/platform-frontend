...
import {
  useGetOrgPaymentConfigs,
  useUpsertOrgPaymentConfig,
  useRemoveOrgPaymentConfig,
...
function ConfigDialog({ orgId, existing, open, onClose }: ConfigDialogProps) {
  ...
  const { mutate, isPending } = useUpsertOrgPaymentConfig(orgId);
...
function ProviderRow({ config, orgId }: ProviderRowProps) {
  const [editOpen, setEditOpen] = useState(false);
  const { mutate: remove, isPending: removing } =
    useRemoveOrgPaymentConfig(orgId);
  ...
  return (
    <>
      <div className="flex items-center gap-4 py-3 border-b border-border/40 last:border-0">
        ...
        <div className="flex-1 min-w-0 grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs text-muted-foreground">
          <div className="flex items-center gap-1.5">
            <KeyRound className="w-3 h-3 shrink-0" />
            <span>
              {config.has_secret_key ? "Secret key set" : "No secret key"}
            </span>
          </div>
          {config.public_key && (
...
export function PaymentConfigSection({
  ...
  const { data, isLoading } = useGetOrgPaymentConfigs(organisationId);
      ...
      <Card className="border-none shadow-lg bg-card/80">
        ...
        <CardContent className="pt-4">
          {isLoading ? (
            ...
            <div>
              {configs.map((c) => (
                <ProviderRow key={c.id} config={c} orgId={organisationId} />
              ))}
            </div>
          )}
        </CardContent>
