  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPayment(json.data as PaymentDetail);
        if (json.batch) setBatch(json.batch as BatchDetail);
        if (json.mandate) setMandate(json.mandate as MandateDetail);
        if (json.approvalRequest)
          setApprovalRequest(json.approvalRequest as ApprovalRequestDetail);
        if (json.approvalActions)
          setApprovalActions(json.approvalActions as ApprovalActionDetail[]);
        if (json.transaction)
          setTransaction(json.transaction as TransactionDetail);
      } catch (error) {
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
            ...
            <DetailRow
              label="Amount"
              value={fmt(Number(payment.amount))}
              bold
            />
            <DetailRow label="Currency" value={payment.currency} />
            <DetailRow
              label="Source Account"
              value={payment.source_account_label ?? payment.source_account}
            />
            <DetailRow
              label="Schedule"
              value={
                payment.schedule === "immediate"
                  ? "Immediate"
                  : payment.schedule === "scheduled"
                    ? "Scheduled"
                    : "Recurring"
              }
            />
            {mandate && (
              <DetailRow label="Frequency" value={mandate.frequency} />
            )}
            {batch && <DetailRow label="Associated Batch" value={batch.name} />}
            {payment.recipient_tag && (
              <DetailRow label="Recipient Tag" value={payment.recipient_tag} />
            )}
            {payment.memo && <DetailRow label="Memo" value={payment.memo} />}
            <DetailRow
              label="Submitted By"
              value={`${payment.submitted_by_name} (${payment.submitted_by_role})`}
            />
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
              <div>
                ...
                <p className="mt-0.5">
                  <Badge
                    className={
                      (
                        statusStyles[approvalRequest.status] ??
                  ...
                  <Badge
                    ...
                    className={
                      approvalRequest.priority === "high"
                        ...
                        : approvalRequest.priority === "critical"
              ...
              <DetailRow
                label="Progress"
                value={`Step ${approvalRequest.current_step} of ${approvalRequest.total_steps}`}
              />
              {approvalRequest.due_by && (
                <DetailRow
                  label="Due By"
                  value={format(
                    new Date(approvalRequest.due_by),
                    "dd MMM yyyy, HH:mm",
                  )}
                />
              )}
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
              ...
              <DetailRow
                label="Amount"
                value={`${transaction.direction === "credit" ? "+ " : "- "}${fmt(Number(transaction.amount))}`}
                bold
              />
              <DetailRow
                label="Counterparty"
...
function DetailRow({
  ...
  return (
    <div>
      ...
      <p className={`mt-0.5 ${bold ? "font-semibold" : "font-medium"}`}>
        {value}
      </p>
    </div>
  );
